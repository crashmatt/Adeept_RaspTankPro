...
def connection_thread():
    ...
    while 1:
        ...
        car_info = client_socket.recv(BUFSIZ).decode()
        if not car_info:
            continue
...
def Info_receive():
    ...
    ADDR = (HOST, INFO_PORT)
    InfoSock = socket.socket(AF_INET, SOCK_STREAM)
...
def socket_connect():	 #Call this function to connect with the server
    global ADDR,client_socket,BUFSIZ,ip_stu,ipaddr
    ...
    SERVER_IP = ip_adr
    SERVER_PORT = 8888  #10223   #Define port serial 
...
def servo_buttons(x,y):
    ...
    def call_lookleft(event):
        global servo_stu
        if servo_stu == 0:
            client_socket.send(('lookleft').encode()) 
            servo_stu = 1

    def call_lookright(event):
        global servo_stu
        if servo_stu == 0:
            client_socket.send(('lookright').encode()) 
            servo_stu = 1

    def call_lookup(event):
        global servo_stu
        if servo_stu == 0:
            client_socket.send(('lookup').encode()) 
            servo_stu = 1
    ...
    def call_lookdown(event):
        ...
        if servo_stu == 0:
            ...
            client_socket.send(('lookdown').encode()) 
            servo_stu = 1
    ...
    def call_loose(event):
        ...
        if servo_stu == 0:
            ...
            client_socket.send(('loose').encode()) 
            servo_stu = 1
